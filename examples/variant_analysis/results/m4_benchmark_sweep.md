...
>
> **Validated 2026-05-31 with HGVS-aware loader** — an HGVS resolver (`agentic_spliceai.splice_engine.utils.HgvsResolver`) was wired into `MutSpliceDBLoader` to correct ~53% of rows where the TSV's `strand` column was wrong (real bug, distinct from the initial position-offset hypothesis). Re-running with the corrected strand annotations gave **identical numbers** (50.7% / 72.1%), so the v4 numbers above are correct as-is, not biased low. Results from the validation re-run: [`mutsplicedb_analysis_v4_resolver_mane_fixed/`](mutsplicedb_analysis_v4_resolver_mane_fixed/).
>
> ⚠️ **Why they are identical is *not* established.** An earlier version of this note claimed the invariance "confirms the consequence-concordance metric is robust to allele orientation." That inference is unsupported: [`04_mutsplicedb_benchmark.py`](../04_mutsplicedb_benchmark.py) resolves strand as `strand = gene_strands.get(gene, v.strand)` — a **GTF gene-name lookup that overrides the record's strand**. Spot-checking the four genes the BACKLOG lists as flipped (MTHFR, POMGNT1, FUBP1, GLUL), the *pre*-resolver run already scored them as `−`. The benchmark was therefore already immune to the TSV strand bug for any gene present in the GTF, and the alleles most likely never differed between the two runs. Demonstrating actual metric robustness would require forcing the raw TSV strand through the scorer. The numbers stand; only the explanation is retracted.
