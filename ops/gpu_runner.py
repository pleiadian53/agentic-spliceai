...
(genomic foundation models, vision, NLP, etc.). Select via ``--model <name>``
on the CLI.
...
Usage from Python::
    ...
    launch(config, output_local=Path("./output/my_run/"), infra=infra)

Usage from CLI: see ``ops/provision_cluster.py`` and ``ops/run_pipeline.py``.
"""
...
# Editable install of the agentic-spliceai package plus the extras needed for
# splice-engine eval on a pod (bio: pysam/pyfaidx/biopython; conservation:
# pyBigWig for phyloP/epigenetic tracks).
_PACKAGE_INSTALL = 'pip install -e ".[bio,conservation]"'
...
class InfraConfig:
    ...
    Data paths are composed from two fields:
      ...
      - ``data_path``: dataset-specific subpath organized as
        ``<source>/<build>`` (e.g., ``mane/GRCh38``, ``ensembl/GRCh37``).
...
def build_setup_lines(infra: InfraConfig) -> list[str]:
    """Build the SkyPilot ``setup`` block shared by all launch paths.

    Installs the agentic-spliceai package (with bio/conservation extras),
    optional interactive tooling, the selected model dependency, and any
    ``extra_setup`` commands. Uses the network-volume pip cache when available
    ...
    setup_lines.append(_PACKAGE_INSTALL)
    ...
    # Experiment tracking + .env loader. Both lightweight; conditional install.
    setup_lines.append(
...
def _derive_job_name(run_command: str) -> str:
    ...
    if match:
        name = match.group(1)
        # Strip numeric prefix (e.g., "13_evaluate_m3_novel" -> "evaluate-m3-novel")
        name = re.sub(r"^\d+_?", "", name)
        name = name.replace("_", "-")
        return f"aspliceai-{name}"
    return "aspliceai-job"
...
def build_skypilot_config(
    ...
    # Extra file mounts (e.g., cached embeddings for training)
...
def stage_data(infra: InfraConfig) -> None:
    ...
    print()
    print("This uploads data once. Future runs use the volume to skip re-upload.")
    print()
    ...
    print("  python ops/run_pipeline.py --execute -- python your_script.py --args")
    print()
